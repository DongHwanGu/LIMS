...
{
    public class CommonCode_Biz : IDisposable
    {
        public void Dispose()
        {
        }
        ...
        /// <returns></returns>
        public DataSet GetMasterCodeList(Dictionary<string, object> dicParam)
        {
            DataSet ds = null;
            using (CommonCode_Dac dac = new CommonCode_Dac())
            {
                ds = dac.GetMasterCodeList(dicParam);
            }
            return ds;
        }
        ...
        public DataSet GetSubCodeList(Dictionary<string, object> dicParam)
        {
            ...
            using (CommonCode_Dac dac = new CommonCode_Dac())
            {
                ds = dac.GetSubCodeList(dicParam);
            }
            return ds;
        }
        ...
        public IntertekBase.IntertekResult SaveCmcodeData(ref IntertekBase.IntertekResult result, Dictionary<string, object> dicParam)
        {
            ...
            try
            {
                using (CommonCode_Dac dac = new CommonCode_Dac())
                {
                    result = dac.SaveCmcodeData(ref result, dicParam);
                    // 하나의 트렌젝션으로 묶기 위함
