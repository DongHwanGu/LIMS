<%@ Page Title="" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="DataInput.aspx.cs" Inherits="UI_Data_DataInput" %>

<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="Server">
    <script>
        var ServiceUrl = "/WebService/UI_Data/DataInput_Service.asmx/";
        var selectRow = {};
        //======================================================
        // Note - 변수
        //======================================================
        ...
        var grd_Master_List = {
            ...
            columns: [
                ...
                { id: "TEST_TEAM_NM", name: "해당팀", field: "TEST_TEAM_NM", cssClass: "cell-align-left", width: 100, sortable: true },
                { id: "STATUS_CD_NM", name: "팀 상태", field: "STATUS_CD_NM", cssClass: "cell-align-center", width: 100, sortable: true, formatter: GDH_StatusBackColor },
        ...
        function grid_init() {
            ...
            function fn_btn_upload(cellvalue, options, rowObject) {
                if (rowObject.WORKSHEET_STATUS_CD == '03' || rowObject.WORKSHEET_STATUS_CD == '90') {
                    return '<input type="button" onclick="fn_test_file_upload(' + (options.rowId - 1) + ');" value="Upload" style="min-width:50px; text-align:center;" class="btn btn-info btn-sm"/>';
                }
                return '-'
            };
            ...
            $grd_Test_Result_List.jqGrid({
                datatype: 'local',
                data: [],
                colNames: ["-", "시험항목", "시험방법", "단위", "검출한계", "스팩(규격)", "레퍼런스", "해당팀", "실무자", "결과", "비고", "파일", "-"],
                colModel: [
                    ...
                    { name: "UNIT_NM", align: "left", width: 100, fixed: true, editable: false },
                    ...
                    { name: "RESULT_BTN", align: "center", width: 80, fixed: true, editable: false, formatter: fn_btn_upload },
        ...
        //======================================================
        $(function () {
            $('#btnSave, #btnSaveAndStatus').click(function () {
                SaveDataInput(this.id);
            });
        ...
        function InitControls() {
            ...
            $('#ddlteamstatus_cd_s').empty();
            $('#ddlteamstatus_cd_s').append(GetCMCODE_Level_New({ cd_major: "0010", cd_level: '11', IsEmpty: true, Text: "ALL", Delete: '01,05,99' }));

            $('#ddlmaster_status_cd').empty();
            $('#ddlmaster_status_cd').append(GetCMCODE_Level_New({ cd_major: "0005", cd_level: '11', IsEmpty: false, Text: "" }));
        }
        function fn_test_file_upload(row) {
            selectRow = $('#grd_Test_Result_List').jqGrid('getGridParam', 'data')[row];
            var strKey = selectRow.REQ_NUM + ',' + selectRow.SAMPLE_ID + ',' + selectRow.TEST_SEQ + ',' + '<%= CkUserId %>';

            fileupload_obj.filemodal = $('#modalfileupload');
            fileupload_obj.multi = true;
            fileupload_obj.page = "DataInput_Test";
        ...
        function SaveDataInputReject() {
            ...
            $.ajax({
                url: ServiceUrl + "SaveDataInputReject",
                type: "POST",
        ...
        // Note - 선택된 담당자 저장.
        //======================================================
        function SaveDataInput(btnID) {
            var btnGb = btnID == 'btnSave' ? 'Save' : 'StatusSave';
            var arrGparam = [];
            var dt = $('#grd_Test_Result_List').jqGrid('getGridParam', 'data');
        ...
        // Note - 마스터 상세.
        //======================================================
        function SetDataInputDetail(row) {
            $('#btnSave').addClass('hidden');
            $('#btnSaveAndStatus').addClass('hidden');
            ...
            // Lab-In / 작성완료 상태
            if (row.STATUS_CD == '02' || row.STATUS_CD == '03') {
                // 시험원 및 관리자
                if (row.CBKLMS_USER_GB == '05' || row.CBKLMS_USER_GB == '99') {
                    $('#btnSave').removeClass('hidden');
                }
                // 팀장 및 관리자
                if (row.CBKLMS_USER_GB == '04' || row.CBKLMS_USER_GB == '99') {
                    $('#btnSave').removeClass('hidden');
                    // 결과값이 다 입력되었다면.
                    if (row.BTN_CLOSED == 'Y') {
                        $('#btnReject').removeClass('hidden');
                        $('#btnSaveAndStatus').removeClass('hidden');
            ...
            var Gparam = {
                ...
                IV_TEST_TEAM: row.TEST_TEAM,
                IV_USER_ID: '<%= CkUserId %>'
            }
            $.ajax({
                url: ServiceUrl + "SetDataInputDetail",
                type: "POST",
                contentType: "application/json; charset=utf-8",
        ...
        // Note - 리스트 조회.
        //======================================================
        function GetDataInputList() {
            var Gparam = {
                IV_START_DT: $('#txtstart_dt_s').val().replaceAll('-', ''),
                IV_END_DT: $('#txtend_dt_s').val().replaceAll('-', ''),
                IV_TEAM_STATUS_CD: $('#ddlteamstatus_cd_s').val(),
                IV_USER_ID: '<%= CkUserId %>'
            };
            $.ajax({
                url: ServiceUrl + "GetDataInputList",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
        ...
        // Note - 업로드 후
        //======================================================
        function fileUploadBinding_Test() {
            var Gparam = {
                IV_REQ_NUM: selectRow.REQ_NUM,
                IV_TEST_TEAM: selectRow.TEST_TEAM,
                IV_USER_ID: '<%= CkUserId %>'
            }
            $.ajax({
                url: ServiceUrl + "SetDataInputDetail",
                ...
                success: function (response) {
                    ...
                    if (jsonData != null) {
                        for (var i = 0; i < jsonData.result.length; i++) {
                            jsonData.result[i].RESULT_VALUE = dt[i].RESULT_VALUE;
                            jsonData.result[i].RESULT_REMARK = dt[i].RESULT_REMARK;
                        }
                        $('#grd_Test_Result_List').jqGrid('clearGridData');
                        $('#grd_Test_Result_List').jqGrid('setGridParam', { data: jsonData.result });
                        $('#grd_Test_Result_List').trigger('reloadGrid');
                ...
                <div class="panel-heading form-horizontal">
                    <div class="panel_serch">
                        <div class="form-group">
                            ...
                            <label for="ddlmasterstatus_cd_s" class="col-md-1 col-xs-4 control-label">팀 상태</label>
                            <div class="col-md-3 col-xs-8">
                                <asp:DropDownList ID="ddlteamstatus_cd_s" runat="server" name="ddlteamstatus_cd_s" CssClass="form-control input-sm" ClientIDMode="Static">
                                </asp:DropDownList>
                            </div>
                        </div>
                    </div>
        ...
        <div class="tab-pane fade" id="detail">
            <div class="panel panel-default">
                <div class="panel-heading" style="min-height:62px">
                    ...
                    <button type="button" class="btn btn-success btn-sm" data-toggle="confirmation" data-popout="true" id="btnSaveAndStatus">
                        <span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;결과 확인
                    </button>
                    ...
                    <div class="col-xs-12 layout_dtl_page">
                        ...
                        <div class="tab-content" style="background-color: #fafafa;">
                            ...
                            <div id="divcomment_log" class="alert alert-dismissible alert-primary" style="background: #fafafa; border: 1px solid #ddd">
                                <button type="button" class="close" data-dismiss="alert">&times;</button>
                                <%--<label for="txtmater_comment" class="control-label ">■ Master Comment</label>
                                <input type="text" class="form-control input-sm" id="txtmater_comment" name="txtreq_key" readonly="readonly" />--%>
                                <label for="txtteam_comment" class="control-label ">■ Comment (View)</label>
                                <input type="text" class="form-control input-sm" id="txtteam_comment" name="txtreq_key" readonly="readonly" />
                            </div>
