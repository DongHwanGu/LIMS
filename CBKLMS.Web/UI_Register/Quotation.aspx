<%@ Page Title="" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="Quotation.aspx.cs" Inherits="UI_Register_Quotation" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" Runat="Server">
    ...
    <script>
        var ServiceUrl = "/WebService/UI_Register/Quotation_Service.asmx/";
        var Register_IN_ServiceUrl = "/WebService/UI_Register/Register_IN_Service.asmx/";
        var Register_Out_ServiceUrl = "/WebService/UI_Register/Register_OUT_Service.asmx/";
        ...
        var grd_Sample_File_List_columns = [
                grd_Sample_File_List_checkbox.getColumnDefinition(),
                { id: "FILE_CNT", name: "-", field: "FILE_CNT", cssClass: "cell-align-center", width: 80, options: fn_grid_optionDatas_sample_cnt(), editor: GDH_SelectEditor, formatter: GDH_SelectFormatter },
                { id: "FILE_TYPE", name: "구분", field: "FILE_TYPE", cssClass: "cell-align-center", width: 80, options: fn_grid_optionDatas('0009', false), editor: GDH_SelectEditor, formatter: GDH_SelectFormatter },
                { id: "FILE_NM", name: "파일명", field: "FILE_NM", cssClass: "cell-align-left", width: 200, sortable: true },
        ...
        var grd_Sample_File_List = {
            ...
            dataView: new Slick.Data.DataView({ inlineFilters: true }),
            options: edit_options,
            columns: grd_Sample_File_List_columns
        };
        ...
        function grid_init() {
            ...
            function fn_btn_use(cellvalue, options, rowObject) {
                if (rowObject.USE_YN == 'Y') {
                    if (rowObject.MASTER_STATUS_CD == '85' || rowObject.MASTER_STATUS_CD == '89') {
                        return rowObject.USE_YN;
                    }
            ...
            $grd_Quotation_Report_List = $("#grd_Quotation_Report_List"),
            ...
            $grd_Quotation_Report_List.jqGrid({
                datatype: 'local',
                data: [],
                colNames: ["견적서 종류", "세금", "파일명", "word", "pdf", "사용"],
                colModel: [
                    { name: "QUOTATION_TYPE_NM", align: "center", width: 150, fixed: true, editable: false },
                    { name: "TAX_TYPE_NM", align: "center", width: 100, fixed: true, editable: false },
                    { name: "FILE_NM", align: "center", width: 150, fixed: true, editable: false },
                    { name: "FILE_WORD_URL", align: "center", width: 150, fixed: true, editable: false, formatter: fn_word_file_link },
                    { name: "FILE_PDF_URL", align: "center", width: 150, fixed: true, editable: false, formatter: fn_pdf_file_link },
        ...
        $(function () {
            ...
            $('#ddlquotation_type').change(function () {
                $('#ddltax_type').val('');
                $('#ddltax_type').attr('disabled', false);
                if ($(this).val() == '02' || $(this).val() == '05') {
                    $('#ddltax_type').attr('disabled', true);
                }
            });
            $('#btnquotation_create').click(function () {
                SaveQuotationCreate();
            });
            $('#btnquotation_email').click(function () {
                mailsend_obj.thismodal = $('#modalMailSend');
                mailsend_obj.page = 'Mail_Quotation';
            ...
            // 저장
            $('#btnSave, #btnSaveApproval, #btnApproval').click(function () {
                SaveRegisterFirst(this.id);
            });
            ...
            // 접수진행 견적취소 처음
            $('#btnRegister, #btnClosed, #btnReset').click(function () {
                var strId = this.id;
                var msg = strId == 'btnRegister' ? '접수 진행을 계속 하시겠습니까?'
                            : strId == 'btnClosed' ? '견적 취소를 진행 하시겠습니까?'
                            : strId == 'btnReset' ? '처음으로 이동 하시겠습니까?' : '';
                Common.Dialog.confirm({
                    content: msg
                    , ok: function () {
                        if (strId == 'btnRegister') {
                            SaveQuotationGoToRegister();
                        }
                        if (strId == 'btnClosed') {
                            SaveQuotationClosed();
                        }
            ...
            $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
                ChangejQGridDesign("#grd_Quotation_Report_List", "");
            });
            $(document).on('focusout', '[role="gridcell"] *', function () {
                $("#grd_Quotation_Report_List").jqGrid('editCell', 0, 0, false);
            });
            // 초기 그리드 바인딩할때 필수..
            $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        ...
        function InitControls() {
            ...
            $('#ddlmasterstatus_cd_s').append(GetCMCODE_Level_New({ cd_major: "0005", cd_level: '11', IsEmpty: true, Text: "ALL", Delete: '01,02,03,04,05,06,07,08,10,11,12,90,91,99' }));
            ...
            $('#ddlquotation_type').empty();
            $('#ddlquotation_type').append(GetCMCODE_Level_New({ cd_major: "0015", cd_level: '11', IsEmpty: false, Text: "" }));
            $('#ddltax_type').empty();
            $('#ddltax_type').append(GetCMCODE_Level_New({ cd_major: "0016", cd_level: '11', IsEmpty: true, Text: "=== Select ===" }));
            $('#ddlextra_type').empty();
            ...
            $('#btnSaveApproval').removeClass('hidden');
            $('#btnApproval').addClass('hidden');
            $('#btnReject').addClass('hidden');
            $('#btnRegister').addClass('hidden');
            $('#btnClosed').addClass('hidden');
            $('#btnReset').addClass('hidden');

            $('#ddlaproval_user_id').attr('disabled', false);
            $('#txtapproval_reamrk').attr('disabled', true);

            $('#div_quotion').addClass('hidden');
        ...
        // Note - 견적서에서 접수로 보내기.
        //======================================================
        function SaveQuotationGoToRegister() {
            var Gparam = {
            ...
            for (var i = 0; i < grd_Sample_List.dataView.getItems().length; i++) {
                ...
                arrGparam.push({
                    IV_REQ_NUM: $('#hidreq_num').val(),
                    IV_SAMPLE_ID: dr.SAMPLE_ID,
                    IV_USER_ID: '<%= CkUserId %>',
                });
            }
            $.ajax({
                url: ServiceUrl + "SaveQuotationGoToRegister",
                type: "POST",
        ...
        function SaveQuotationClosed() {
            ...
            $.ajax({
                url: ServiceUrl + "SaveQuotationClosed",
                type: "POST",
                contentType: "application/json; charset=utf-8",
        ...
        function SaveQuotationReset() {
            ...
            $.ajax({
                url: ServiceUrl + "SaveQuotationReset",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
        ...
        function fn_btn_use_click(row) {
            var selectRow = $('#grd_Quotation_Report_List').jqGrid('getGridParam', 'data')[row];

            var Gparam = {
                IV_REQ_NUM: $('#hidreq_num').val(),
                IV_QUOTATION_ID: selectRow.QUOTATION_ID,
        ...
        // Note - 견적서 생성
        //======================================================
        function SaveQuotationCreate() {
            var Gparam = {
                IV_REQ_NUM: $('#hidreq_num').val(),
                IV_QUOTATION_TYPE: $('#ddlquotation_type').val(),
                IV_TAX_TYPE: $('#ddltax_type').val(),
                IV_FILE_NM : '',
                IV_FILE_WORD_URL: '',
                IV_FILE_PDF_URL: '',
                IV_USER_ID: '<%= this.CkUserId %>'
            ...
            if (Gparam.IV_QUOTATION_TYPE != '02') {
                if (Gparam.IV_TAX_TYPE == '') {
                    fn_info('과세 유무를 선택해 주세요.');
                    return;
                }
            }

            $.ajax({
                url: ServiceUrl + "SaveQuotationCreate",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
        ...
        // Note - 견적서 발행목록.
        //======================================================
        function GetQuotationFileList() {
            var Gparam = {
                IV_REQ_NUM: $('#hidreq_num').val(),
                IV_USER_ID: '<%= CkUserId %>'
            }
            $.ajax({
                url: ServiceUrl + "GetQuotationFileList",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
                success: function (response) {
                    ...
                    if (jsonData != null) {
                        $('#grd_Quotation_Report_List').jqGrid('clearGridData');
                        $('#grd_Quotation_Report_List').jqGrid('setGridParam', { data: jsonData.result });
                        $('#grd_Quotation_Report_List').trigger('reloadGrid');
                    }
                },
                beforeSend: function () {
                    $('.wrap-loading').removeClass('display-none');
        ...
        function SetDueReportDate() {
            var type_day = parseInt($('#ddlreg_service_type').val());
        ...
        function ClearObjectAll() {
            ...
            pm_req_test = [];
            pm_req_test_package = [];
            pm_req_quotation_approval = [];
        }

        //======================================================
        // Note - 승인 진행
        //======================================================
        function SaveQuotationAprooval(btnId) {
            var Gparam = {
                IV_REQ_NUM: $('#hidreq_num').val(),
                IV_APPROVAL_ID: $('#hidapproval_id').val(),
                IV_APPROVAL_STATUS_CD: '',
                IV_REMARK: $('#txtapproval_reamrk').val(),
                IV_USER_ID: '<%= CkUserId %>',
            };
            if (btnId == 'btnApproval') {
                Gparam.IV_APPROVAL_STATUS_CD = '82'
            }
            else {
                Gparam.IV_APPROVAL_STATUS_CD = '88'
            }

            $.ajax({
                url: ServiceUrl + "SaveQuotationAprooval",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
        ...
        function GetCurrency_Amt(currency_cd) {
            ...
            $.ajax({
                url: Register_IN_ServiceUrl + "GetCurrency_Amt",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
        ...
        function SaveModalPackageTestList() {
            ...
            $.ajax({
                url: Register_IN_ServiceUrl + "SaveModalPackageTestList_GetDetail",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
        ...
        function GetModalPackageTestList() {
            ...
            $.ajax({
                url: Register_IN_ServiceUrl + "GetModalPackageTestList",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
        ...
        function SaveModalContactList() {
            ...
            for (var i = 0; i < gridData.length; i++) {
                var ModalList = {
                    ...
                    CONTACT_TELHP: grd_Modal_Contact_List.grid.getDataItem(gridData[i])["CONTACT_TELHP"],
        ...
        function GetModalContactList() {
            ...
            $.ajax({
                url: Register_IN_ServiceUrl + "GetModalContactList",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
        ...
        function GetRegisterDetail(row) {
            ...
            $('#btnSaveApproval').addClass('hidden');
            $('#btnApproval').addClass('hidden');
            $('#btnReject').addClass('hidden');
            $('#btnRegister').addClass('hidden');
            $('#btnClosed').addClass('hidden');
            $('#btnReset').addClass('hidden');
            $('#btnquotation_create').addClass('hidden');
            $('#btnquotation_email').addClass('hidden');
            $('#div_quotion').addClass('hidden');

            $('#hidapproval_id').val(row.APPROVAL_ID);
            $('#ddlaproval_user_id').val(row.APPROVAL_USER_ID);
            $('#txtapproval_reamrk').val(row.APPROVAL_REMARK);
            if (row.STATUS_CD == '80') {
                $('#btnSave').removeClass('hidden');
                $('#btnSaveApproval').removeClass('hidden');
                $('#ddlaproval_user_id').attr('disabled', false);
                ...
                if (row.STATUS_CD == '81') { // 견적 승인중
                    if (row.APPROVAL_STATUS_CD == '01' && (row.APPROVAL_USER_ID == '<%= CkUserId %>' || '<%= CkUserId %>' == '100000000' || '<%= CkUserId %>' == '100000003')) {
                        $('#btnApproval').removeClass('hidden');
                        $('#btnReject').removeClass('hidden');
                        $('#ddlaproval_user_id').attr('disabled', true);
                        $('#txtapproval_reamrk').attr('disabled', false);
                    }
                }
                if (row.STATUS_CD == '82' || row.STATUS_CD == '83') { // 견적 승인완료 발행완료
                    $('#ddlaproval_user_id').attr('disabled', true);
                    $('#txtapproval_reamrk').attr('disabled', true);
                    $('#btnquotation_create').removeClass('hidden');
                    $('#btnquotation_email').removeClass('hidden');
                    $('#div_quotion').removeClass('hidden');
                }
                if (row.STATUS_CD == '84') { // 견적 발송완료
                    $('#ddlaproval_user_id').attr('disabled', true);
                    $('#txtapproval_reamrk').attr('disabled', true);
                    $('#btnquotation_create').removeClass('hidden');
                    $('#btnquotation_email').removeClass('hidden');
                    $('#btnRegister').removeClass('hidden');
                    $('#btnClosed').removeClass('hidden');
                    $('#btnReset').removeClass('hidden');
                    $('#div_quotion').removeClass('hidden');
                }
                if (row.STATUS_CD == '85') { // 견적 접수완료
                    $('#ddlaproval_user_id').attr('disabled', true);
                    $('#txtapproval_reamrk').attr('disabled', true);
                    $('#div_quotion').removeClass('hidden');
                }
                if (row.STATUS_CD == '88') { // 견적 반려
                    $('#btnSave').removeClass('hidden');
                    $('#btnSaveApproval').removeClass('hidden');
                    $('#ddlaproval_user_id').attr('disabled', false);
                    $('#txtapproval_reamrk').attr('disabled', true);
                }
                if (row.STATUS_CD == '89') { // 견적 취소
                    $('#ddlaproval_user_id').attr('disabled', true);
                    $('#txtapproval_reamrk').attr('disabled', true);
                    $('#div_quotion').removeClass('hidden');
                }
            ...
            $.ajax({
                url: Register_Out_ServiceUrl + "GetRegisterDetail",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
        ...
        // Note - 견적서 진행
        //======================================================
        function SaveRegisterFirst(btnId) {
            ClearObjectAll();
            ...
            if (btnId == 'btnSaveApproval') {
                if ($('#ddlaproval_user_id').val() == '') {
                    fn_info('승인자를 설정해 주세요.');
                    return;
                }
            ...
            $.ajax({
                url: Register_Out_ServiceUrl + "SaveRegisterFirst",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({
                    "pm_req": pm_req, "pm_req_file": pm_req_file, "pm_req_customer": pm_req_customer, 'pm_req_contact': pm_req_contact,
                    "pm_req_sample": pm_req_sample, "pm_req_sample_file": pm_req_sample_file, "pm_req_test": pm_req_test, "pm_req_sample_test": pm_req_sample_test
                }),
                dataType: "json",
                success: function (response) {
                    var result = response.d;
                    ...
                    if (result.OV_RTN_CODE == "0") {
                        if (btnId == 'btnSaveApproval') {
                            SaveRegisterFirstApproval(result.OV_RTN_MSG);
                        } else if (btnId == 'btnApproval') {
                            SaveQuotationAprooval('btnApproval');
                        }
                        else {
                            $('#ulTabs li:eq(0) a').tab('show');
                            GetRegisterList();
        ...
        function SaveRegisterFirstApproval(req_num) {
            pm_req_quotation_approval.push({
                IV_REQ_NUM: req_num,
                IV_APPROVAL_GB: '01',
                IV_APPROVAL_USER_ID: $('#ddlaproval_user_id').val(),
                IV_USER_ID: '<%= CkUserId %>',
            });
            $.ajax({
                url: ServiceUrl + "SaveRegisterFirstApproval",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "pm_req_quotation_approval": pm_req_quotation_approval }),
                dataType: "json",
                success: function (response) {
                    var result = response.d;
        ...
        function GetSampleModal(row) {
            ...
            $('#txtsample_file_url').val(row.FILE_URL);
            $('#txtsample_file_nm').val(row.FILE_NM);
        ...
        function SetTestPackageGrid() {
            ...
            $('.slick-viewport').on('blur', '.editor-select', function (e) {
                window.setTimeout(function () {
                    Slick.GlobalEditorLock.commitCurrentEdit();
                }, 0);
            });
        ...
        $.ajax({
            url: Register_IN_ServiceUrl + "GetModalUserList",
            type: "POST",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify({ "Gparam": Gparam }),
            dataType: "json",
    ...
    function GetModalCustomerList(cust_gb) {
        ...
        var Gparam = {
            ...
            $.ajax({
                url: Register_IN_ServiceUrl + "GetModalCustomerList",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
            ...
            if (clickName != '') {
                switch (clickName) {
                    case "txtreg_file_nm":
                        ...
                        break;
                    case "txtreg_file_nm":
                        linkUrl = $('#txtsample_file_url').val();
                        break
                    default:
                        break;
                }
        ...
        <div class="tab-pane fade" id="detail">
            <div class="panel panel-default">
                <div class="panel-heading">
                    ...
                    <button type="button" class="btn btn-warning btn-sm" data-toggle="confirmation" data-popout="true" id="btnSave">
                        <span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;초안
                    </button>
                    <button type="button" class="btn btn-success btn-sm" data-toggle="confirmation" data-popout="true" id="btnSaveApproval">
                        <span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;저장 & 승인요청
                    </button>

                    <button type="button" class="btn btn-success btn-sm" data-toggle="confirmation" data-popout="true" id="btnApproval">
                        <span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;승인
                    </button>
                    <button type="button" class="btn btn-danger btn-sm" data-toggle="confirmation" data-popout="true" id="btnReject">
                        <span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;반려
                    </button>

                    <button type="button" class="btn btn-default btn-sm" data-toggle="confirmation" data-popout="true" id="btnReset">
                        <span class="glyphicon glyphicon-refresh"></span>&nbsp;처음으로
                    </button>
                    <button type="button" class="btn btn-danger btn-sm" data-toggle="confirmation" data-popout="true" id="btnClosed">
                        <span class="glyphicon glyphicon-trash"></span>&nbsp;견적 취소
                    </button>
                    <button type="button" class="btn btn-success btn-sm" data-toggle="confirmation" data-popout="true" id="btnRegister">
                        <span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;접수 진행
                    </button>
                </div>
                    ...
                    <div class="col-xs-12 layout_dtl_page">
                        ...
                        <div class="tab-content" style="background-color: #fafafa;">
                            ...
                            <fieldset class="scheduler-border table-responsive">
                                <legend class="scheduler-border">승인자 설정</legend>
                                <div class="form-group col-md-4 col-xs-12">
                                    <input type="hidden" id="hidapproval_id" />
                                    <asp:DropDownList ID="ddlaproval_user_id" runat="server" name="ddlaproval_user_id" CssClass="form-control input-sm" ClientIDMode="Static">
                                        <asp:ListItem Text="=== Select ===" Value=""></asp:ListItem>
                                        <asp:ListItem Text="이응용 (EungYoung Lee)" Value="100000343"></asp:ListItem>
                                        <asp:ListItem Text="유환우 (Hwanwoo Yoo)" Value="100000277"></asp:ListItem>
                                    </asp:DropDownList>
                                </div>
                                <div class="form-group col-md-8 col-xs-12">
                                    <input type="text" class="form-control input-sm" id="txtapproval_reamrk" name="txtapproval_reamrk"  placeholder="" />
                                </div>
                            </fieldset>

                            <%--견적서 발행 하기.--%>
                            <fieldset class="scheduler-border" id="div_quotion">
                                <legend class="scheduler-border">견적서 발행</legend>
                                <table style="margin-bottom: 10px;">
                                    ...
                                    <tr>
                                        <td></td>
                                        <td>
                                            <asp:DropDownList ID="ddlquotation_type" runat="server" name="ddlquotation_type" CssClass="form-control input-sm" ClientIDMode="Static" Style="width: 150px; margin-bottom: 2px; margin-right: 10px;">
                                            </asp:DropDownList>
                                        </td>
                                        <td>
                                            <asp:DropDownList ID="ddltax_type" runat="server" name="ddltax_type" CssClass="form-control input-sm" ClientIDMode="Static" Style="width: 150px; margin-bottom: 2px; margin-right: 10px;">
                                            </asp:DropDownList>
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-success btn-sm" data-toggle="confirmation" data-popout="true" id="btnquotation_create">
                                                <span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;발행
                                            </button>
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-warning btn-sm" data-toggle="confirmation" data-popout="true" id="btnquotation_email">
                                ...
                                <div style="border-top: 2px solid #555;">
                                    <table id="grd_Quotation_Report_List"></table>
                                </div>
                            </fieldset>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <div class="input-group mgn0 hidden">
                                                        <div class="input-group-btn">
                            ...
                            <fieldset class="scheduler-border table-responsive">
                                ...
                                <div class="form-group col-md-3 col-xs-12 hidden">
                                    <label for="ddltestcurreny_cd" class="control-label">■ 통화 기준 : <span class="ddltestcurreny_cd_com"></span></label>
                                    <br />
                                    <label for="ddltestcurreny_cd" class="control-label">■ 환율 적용 : <span class="chkexchange_rate_com"></span></label>
                                    <br />
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group col-md-3 col-xs-12">
                        <label for="txtsample_nm" class="control-label">■ 성적서 번호</label>
