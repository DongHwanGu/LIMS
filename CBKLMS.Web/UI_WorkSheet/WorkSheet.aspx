<%@ Page Title="" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="WorkSheet.aspx.cs" Inherits="UI_WorkSheet_WorkSheet" %>

<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="Server">
    <script>
        var ServiceUrl = "/WebService/UI_WorkSheet/WorkSheet_Service.asmx/";
        //======================================================
        // Note - 변수
        //======================================================
        ...
        var grd_Master_List = {
            ...
            columns: [
                ...
                { id: "MASTER_STATUS_CD_NM", name: "상태", field: "MASTER_STATUS_CD_NM", cssClass: "cell-align-center", width: 100, sortable: true },
                { id: "TEST_TEAM_NM", name: "해당팀", field: "TEST_TEAM_NM", cssClass: "cell-align-left", width: 100, sortable: true },
                { id: "STATUS_CD_NM", name: "팀 상태", field: "STATUS_CD_NM", cssClass: "cell-align-center", width: 100, sortable: true },
                ...
                { id: "SAMPLE_NM_LIST", name: "시료명", field: "SAMPLE_NM_LIST", cssClass: "cell-align-left", width: 100, sortable: true },
                { id: "SAMPLE_CNT_LIST", name: "수량", field: "SAMPLE_CNT_LIST", cssClass: "cell-align-left", width: 100, sortable: true },
        ...
        function grid_init() {
            ...
            // 담당자 설정
            //-------------------------------------------------------
            $grd_Test_Contact_List = $("#grd_Test_Contact_List"),
            ...
            $grd_Test_Contact_List.jqGrid({
                datatype: 'local',
                data: [],
                colNames: ["-", "시험항목", "시험방법", "검출한계", "스팩(규격)", "레퍼런스", "해당팀", "실무자", "hid_팀"],
                colModel: [
                    ...
                    {
                        name: "TEST_CONTACT_USER_ID", align: "center", width: 150, fixed: true, editable: true, resizable: true, edittype: "select", formatter: "select"
                    },
                    { name: "TEST_TEAM", align: "center", width: 100, fixed: true, editable: false, hidden:true },

                ],
                beforeEditCell: function (id, name, val, iRow, iCol) {
                    if (name == 'TEST_CONTACT_USER_ID') {
                        var test_team = $(this).jqGrid('getRowData', id).TEST_TEAM;
                        var listdata = ":=== Select ===;" + GetContactList(test_team);
                        $(this).jqGrid('setColProp', name, { editoptions: { value: listdata.toString() } });
                    }
                },
        ...
        $(function () {
            ...
            $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
                ChangejQGridDesign("#grd_Test_Contact_List", "");
            });
            $(document).on('focusout', '[role="gridcell"] *', function () {
                $("#grd_Test_Contact_List").jqGrid('editCell', 0, 0, false);
            });
        ...
        function InitControls() {
            ...
            $('#ddlteamstatus_cd_s').append(GetCMCODE_Level_New({ cd_major: "0010", cd_level: '11', IsEmpty: true, Text: "ALL", Delete: '03,04,05,90,91,99' }));
        ...
        // Note - 선택된 담당자 저장.
        //======================================================
        function SaveWorkSheet(btnID) {
            var btnGb = btnID == 'btnSave' ? 'Save' : 'StatusSave';
            var arrGparam = [];
            var dt = $('#grd_Test_Contact_List').jqGrid('getGridParam', 'data');
            ...
            for (var i = 0; i < dt.length; i++) {
                if ($.isNumeric(dt[i].TEST_CONTACT_USER_ID)) {
                    arrGparam.push({
                        IV_REQ_NUM: dt[i].REQ_NUM,
                        IV_SAMPLE_ID: dt[i].SAMPLE_ID,
                        IV_TEST_SEQ: dt[i].TEST_SEQ,
                        IV_TEST_CONTACT_USER_ID: dt[i].TEST_CONTACT_USER_ID,
                        IV_TEST_TEAM : $('#hidtest_team').val(),
                        IV_USER_ID: '<%= this.CkUserId %>'
                    });
                }
        ...
        // Note - 시험원 리스트 가져오기.
        //======================================================
        function GetContactList(test_team) {
            var Gparam = {
                IV_TEST_TEAM: test_team,
            }
            var options = [];

            $.ajax({
                url: ServiceUrl + "GetContactList",
                type: "POST",
                contentType: "application/json; charset=utf-8",
        ...
        // Note - 마스터 상세.
        //======================================================
        function SetWorkSheetDetail(row) {
            ...
            $.ajax({
                url: ServiceUrl + "SetWorkSheetDetail",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
                success: function (response) {
                    ...
                    if (jsonData != null) {
                        $('#grd_Test_Contact_List').jqGrid('clearGridData');
                        $('#grd_Test_Contact_List').jqGrid('setGridParam', { data: jsonData.result });
                        $('#grd_Test_Contact_List').trigger('reloadGrid');
                    }
                },
                beforeSend: function () {
                    $('.wrap-loading').removeClass('display-none');
        ...
        function GetWorkSheetList() {
            ...
            $.ajax({
                url: ServiceUrl + "GetWorkSheetList",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ "Gparam": Gparam }),
                dataType: "json",
            ...
            <div class="panel panel-default">
                <div class="panel-heading" style="height:62px;">
                    ...
                    <button type="button" class="btn btn-success btn-sm" data-toggle="confirmation" data-popout="true" id="btnSaveAndStatus">
                        <span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;Save & Lab-IN
                    </button>
                </div>
                <div class="panel-body form-horizontal">
                    ...
                    <div class="col-xs-12 layout_dtl_page">
                        ...
                        <div class="tab-content" style="background-color: #fafafa;">
                            ...
                            <%--시료 및 테스트 정보--%>
                            <fieldset class="scheduler-border">
                                <legend class="scheduler-border">실무자 설정</legend>
                                <div style="border-top:2px solid #555;">
                                    <table id="grd_Test_Contact_List"></table>
                                </div>
                            </fieldset>
